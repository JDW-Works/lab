...
// 計算符合條件的資料筆數
$countQuery = "SELECT COUNT(*) AS cnt FROM PumpData WHERE pd_name LIKE ?";
...
// 取得目前頁面的資料
$query = "SELECT * FROM PumpData WHERE pd_name LIKE ? ORDER BY (SELECT NULL) OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
...
<form method="get" class="search-box">
    <input type="text" name="pd_name" value="<?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?>" placeholder="搜尋抽水機名稱">
...
if ($totalPages > 0):
    if ($page > 1) {
        echo '<a href="?page=' . ($page - 1) . '&pd_name=' . urlencode($search) . '" class="prev">&laquo; 上一頁</a>';
    } else {
    ...
    for ($i = $start; $i <= $end; $i++) {
        ...
        echo '<a href="?page=' . $i . '&pd_name=' . urlencode($search) . '"' . $active . '>' . $i . '</a>';
    }
    if ($end < $totalPages) {
        echo '<span class="ellipsis">...</span>';
    }
    ...
    if ($page < $totalPages) {
        echo '<a href="?page=' . ($page + 1) . '&pd_name=' . urlencode($search) . '" class="next">下一頁 &raquo;</a>';
    } else {
